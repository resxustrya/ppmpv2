...
<div class="signup-box">
    <div class="logo">
        <small>Edit Expense Code</small>
    </div>
    <div class="card">
        <div class="body">
            <form id="{{ asset('edit/uacs') }}" method="POST">
                <div class="input-group">
                    <span class="input-group-addon">
                        <i class="glyphicon glyphicon-text-color"></i>
                    </span>
                    <div class="form-line">
                    <input type="text" class="form-control" name="uacs"  placeholder="Enter UACS name" value="{{ $uacs->expense_name }}" required>
                    </div>
                </div>
                <div class="input-group">
                    <label for="oneline">One line ?</label>
                    <div class="form-inline">
                        <input type="radio" name="oneline" {{ $uacs->oneline == 1 ? ' checked' : '' }} value="1">Yes</input>
                        <input type="radio" name="oneline" {{ $uacs->oneline == 0 ? ' checked' : '' }} value="0">No</input>
                    </div>
                </div>
                <div class="input-group">
                    <label for="oneline">Placement Order</label>
                    <input type="number" name="order" placeholder="Order" value="{{ $uacs->order }}">
                </div>
