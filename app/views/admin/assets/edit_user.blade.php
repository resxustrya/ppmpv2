...
<div class="signup-box">
    ...
    <div class="card">
        <div class="body">
            <form id="{{ asset('edit/user') }}" method="POST">
                    ...
                    <div class="form-line">
                        <select name="division" class="form-control">
                            ...
                            @foreach(DB::table('division')->get() as $division)
                                <option value="{{ $division->id }}" {{ $user->division == $division->id ? ' selected' : ' ' }}>{{ $division->description }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="form-line">
                        <select name="section" class="form-control">
                            ...
                            @foreach(DB::table('section')->orderBy('description','ASC')->get() as $section)
                                <option value="{{ $section->id }}" {{ $section->id == $user->section ? ' selected' : ' ' }}>{{ $section->description }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                 <div class="input-group">
                    ...
                    <div class="form-line">
                        <select name="user_type" class="form-control">
                            ...
                            <option value="0" {{ 0 == $user->user_priv ? ' selected' : ' ' }}>Section User</option>
                            <option value="1111" {{ 1111 == $user->user_priv ? ' selected' : ' ' }}>Division level user</option>
                        </select>
                    </div>
                </div>
